document.addEventListener('DOMContentLoaded', function () {
  ...
  const getRcwData = async () => {
    ...
    formData.append("apikey", rcwApiKey);
  ...
  const rcwSetTitle = (title, times = []) => {
    if (title == 1) {
      rcwBodyTitle.innerHTML = 'გნებავთ ჩვენ გადმოგირეკავთ?';
      rcwBodyTitleSub.innerHTML = 'მიუთითეთ თქვენი ნომერი და ჩვენ დაგიკავშირდებით 1 წუთის განმავლობაში!';
    } else if (title == 2) {
      rcwBodyTitle.innerHTML = 'უი, სამუშაო საათები დასრულებულია';
      rcwBodyTitleSub.innerHTML = 'დაგირეკავთ შერჩეულ დროს';
    } else if (title == 3) {
      rcwBodyTitle.innerHTML = 'დღეს ვისვენებთ';
      rcwBodyTitleSub.innerHTML = 'დაგიკავშირდებით შერჩეულ დროს';
    } else if (title == 4) {
      rcwBodyTitle.innerHTML = `შესვენება ${times[0]}-დან ${times[1]}-მდე`;
      rcwBodyTitleSub.innerHTML = 'დაგიკავშირდებით შერჩეულ დროს';
    }
  }
  ...
  const sendRcwData = async () => {
    ...
    if (rcwCallMode == 0) {
      time = rcwGetFormattedDateTime(new Date(), 60);
    } else {
    ...
    try {
      ...
      if (data.error == "spam") {
        var button = document.getElementById('rcw-button-submit');
